...
type TroopParamCarrier = { troops?: number };

function hasTroops(
  params: UnitParamsMap[keyof UnitParamsMap],
): params is TroopParamCarrier {
  return typeof params === "object" && params !== null && "troops" in params;
}
...
export class PlayerImpl implements Player {
  ...
  private _populationConversionRemainder = 0;
  ...
  updateMilitaryComposition(conversionRate: number): void {
    ...
    const rawCapacity = total * conversionRate;
    let convertibleUnits = Math.floor(rawCapacity);
    let fractionalCapacity =
      rawCapacity - convertibleUnits + this._populationConversionRemainder;

    if (fractionalCapacity >= 1) {
      const extraUnits = Math.floor(fractionalCapacity);
      convertibleUnits += extraUnits;
      fractionalCapacity -= extraUnits;
    }
    ...
    if (reserve < desiredReserve) {
      ...
      const convertible = Math.min(needed, convertibleUnits, civilian);
      if (convertible > 0) {
      ...
      const convertible = Math.min(surplus, convertibleUnits);
      if (convertible > 0) {
    ...
    const unusedCapacity = Math.max(0, convertibleUnits - converted);
    this._populationConversionRemainder = fractionalCapacity + unusedCapacity;

    if (this._populationConversionRemainder >= 1) {
      this._populationConversionRemainder -= Math.floor(
        this._populationConversionRemainder,
      );
    }
  ...
  buildUnit<T extends keyof UnitParamsMap>(
    ...
    const b = new UnitImpl(
      ...
      this.mg.nextUnitID(),
      this,
      params as AllUnitParams,
    );
    this._units.push(b);
    this.recordUnitConstructed(type);
    this.removeGold(cost);
    const troopCost = hasTroops(params) ? (params.troops ?? 0) : 0;
    this.removeTroops(troopCost);
