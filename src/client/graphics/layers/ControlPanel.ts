...
export class ControlPanel extends LitElement implements Layer {
  ...
  @state()
  private _baseCapBonus: number = 0;

  @state()
  private _baseRegenBonusPercent: number = 0;
  ...
  tick() {
    ...
    const trainingDuration = this.game.config().militaryBaseTrainingDuration();
    const batchSize = this.game.config().militaryBaseTrainingBatchSize();
    const basePerTick =
      trainingDuration > 0 ? (batchSize / trainingDuration) * activeBases : 0;
    const baseCapBonus = this.game
      .config()
      .militaryBaseSoldierCapBonus(activeBases);
    const regenMultiplier = this.game
      .config()
      .militaryBaseRegenMultiplier(activeBases);
    ...
    this._baseCapBonus = baseCapBonus;
    this._baseRegenBonusPercent = Math.max(0, (regenMultiplier - 1) * 100);
    this.requestUpdate();
  }
    ...
    return html`
      ...
      >
        <div class="block bg-black/30 text-white mb-4 p-2 rounded">
          ...
          <div class="flex justify-between text-xs sm:text-sm text-white/80">
            <span>${translateText("control_panel.cap_bonus")}:</span>
            <span translate="no">+${renderTroops(this._baseCapBonus)}</span>
          </div>
          <div class="flex justify-between text-xs sm:text-sm text-white/80">
          ...
          <div class="flex justify-between text-xs sm:text-sm text-sky-300">
            <span>${translateText("control_panel.regen_bonus")}:</span>
            <span translate="no"
              >+${this._baseRegenBonusPercent.toFixed(0)}%</span
            >
          </div>
          <div
