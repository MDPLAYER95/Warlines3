...
export class DefensePostPanel extends LitElement implements Layer {
  ...
  private setGarrisonTarget(targetFraction: number) {
    ...
    } else if (target < this.garrison) {
      const withdrawAmount = this.garrison - target;
      if (withdrawAmount <= 0) {
        return;
      }
      this.eventBus.emit(
        new SendWithdrawDefensePostTroopsIntentEvent(
          this.unitId,
          withdrawAmount,
        ),
      );
  ...
  private renderFillButton(fraction: number, highlightFraction: number) {
    ...
    const disabled = this.maxGarrison <= 0;
    return html`
      <button
        type="button"
  ...
  render() {
    ...
    if (this.maxGarrison > 0) {
      const currentFraction = this.garrison / this.maxGarrison;
